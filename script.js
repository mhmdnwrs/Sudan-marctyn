...
const initialData = {
  ...
  favorites: {} // تخزين المفضلة لكل مستخدم بصيغة: { userId: [prodId1, prodId2] }
};
...
function register() {
  ...
  if (name === 'أنس') {
    return toast('هذا الاسم محجوز لمدير النظام');
  }
...
function updateProfile() {
  ...
  const addProdBtn = document.getElementById('addProductTopBtn');
  const addMarketBtn = document.getElementById('addMarketTopBtn');
  ...
  if (addProdBtn) addProdBtn.style.display = admin ? 'inline-block' : 'none';
  if (addMarketBtn) addMarketBtn.style.display = admin ? 'inline-block' : 'none';
}
